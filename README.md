...
### 2. Mobile Deployment: Responsive Mobile Web App (PWA) vs. Standalone React Native Binaries
* **Trade-off & Limitation**: Hosting live links for native mobile binaries (.apk for Android / .ipa for iOS) is not possible on web deployment platforms like Vercel, which host web servers and APIs rather than compiled native mobile binaries. Furthermore, App Store submissions incur 24-72 hour manual review delays and require developer portal subscriptions ($99/year Apple Developer Program).
* **Design Decision**: Built as a responsive, mobile-first Web Application (Progressive Web App) to ensure zero user drop-off friction. For a 1-to-3-day private event, arriving guests at an airport or drivers in cars can instantly tap a link or scan a QR code to open their role-specific experience inside mobile Safari or Chrome without downloading a 50MB app store binary.
* **Native Compilation Paths**:
  * **Option A (Capacitor PWA Shell)**: Running `npx cap add android` / `npx cap add ios` packages the build directly into native `.apk` and `.ipa` installer packages.
  * **Option B (React Native Expo)**: All core hooks (`useAuth`, `useSSE`, `useLocation`), API handlers, matching algorithms, and TypeScript interfaces in `/src` are decoupled and ready to be imported into a React Native Expo project.
